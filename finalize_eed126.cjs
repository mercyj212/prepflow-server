...
async function main() {
  try {
    ...
    const csDeptId = new mongoose.Types.ObjectId('69e5e34f2eeaa5bffac98e94');
    ...
    // Format 100 user questions cleanly
    const formattedUserQuestions = rawUserQuestions.map(q => {
    ...
    // Generate 100 PDF questions using textbook notes context
    const finalModel = getGenerativeModel("gemini-2.5-flash", {
    ...
    console.log('Generating remaining curriculum questions...');
    const pdfQsPrompt = `
Generate EXACTLY 100 unique multiple-choice questions for EED 126 Entrepreneurship Education.
RULES:
1. Each question must have 4 options, exactly 1 isCorrect: true.
2. All 4 options MUST be equal in word count and character length.
3. Make distractors plausible and realistic business concepts.
4. Include explanations.
`;

    const res = await callWithRetry(() => finalModel.generateContent([{ text: pdfQsPrompt }]));
    const pdfQuestionsRaw = JSON.parse(res.response.text().trim());

    const pdfQuestions = pdfQuestionsRaw.map(q => ({
      ...q,
    ...
    const totalQuestions = [...formattedUserQuestions, ...pdfQuestions];
    console.log(`Total Combined Questions: ${totalQuestions.length}`);
    ...
    console.log(`\n🎉 EED 126 COMPLETED SUCCESSFULLY! Total Questions Saved: ${totalQuestions.length} | Access granted to ${approvedStudents.length} approved users.`);
    process.exit(0);
