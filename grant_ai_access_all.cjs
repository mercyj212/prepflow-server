...
mongoose.connect('mongodb+srv://prepflow:Prepflow4801@cluster0.vjeopgu.mongodb.net/prepflow?retryWrites=true&w=majority').then(async () => {
  ...
  // Find all AI and related courses
  ...
  console.log(`Found ${aiCourses.length} AI & related courses:`, aiCourses.map(c => c.title));
  ...
  console.log(`Granted access to ${granted} new AI/ML/Python courses.`);
  process.exit();
}).catch(console.error);
